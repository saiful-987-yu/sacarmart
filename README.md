...
**Header naming rule**: every `products` sheet column name should be lowercase with underscores instead of spaces (`sku`, `name`, `category`, `category_priority`, `category_image`, `sub_category`, `sub_category_priority`, `sub_category_image`, `buying_price`, `price`, `discount_price`, `points`, `Stock`, `Buffer`, `image_url`, `description`, `best_selling`, `best_selling_priority`, `new_arrival`, `new_arrival_priority`, `ready_to_eat`, `ready_to_eat_priority`, `baby_care`, `baby_care_priority`, `value_combo`, `value_combo_priority`, `time_based_essentials`, `active`, `supplier_name`, `barcode`, ...). The app reads columns by header **name**, not position — so columns can be reordered freely, and new columns can be added anytime without touching any code (see the Admin Editing section below for how new columns show up automatically in Advanced Edit).
...
- **Category / Sub-category ordering**: `category_priority` controls the order of the top category chips, sidebar list, Featured Categories, and All Categories page (lower number shows first; categories without a priority value sort after ones that have it). `sub_category_priority` controls the order of the sub-category chips shown after picking a category, the same way. Both use the shared `sortByGroupPriority()` helper in `script.js`. `sub_category_image` is present in the Sheet but intentionally unused — there is no sub-category image display anywhere in the app yet.
- **Four more Sheet-driven Home sections**, using the exact same flag+priority system as Best Selling/New Arrival — set `TRUE`/`FALSE` plus a priority number, no code changes needed:
  - **Ready-to-Eat / Quick & Easy** — `ready_to_eat` / `ready_to_eat_priority`
  - **Baby Care** — `baby_care` / `baby_care_priority`
  - **Value Combo / Saver Bundles** — `value_combo` / `value_combo_priority`

  All four are global Home sections — they show whether the customer is logged in or not, same as Best Selling/New Arrival, and stay hidden automatically when no product has the flag set.
- **Time-Based Essentials** — a single Sheet column, `time_based_essentials`, holds three comma-separated `TRUE`/`FALSE` values in a fixed order: **Morning, Afternoon, Evening** (e.g. `TRUE, FALSE, TRUE` = show in Morning and Evening, not Afternoon). There is no priority column, and no separate `morning_essentials` / `afternoon_essentials` / `evening_essentials` columns. The Home Page reads the visitor's current local device time and shows one section — titled "Morning Essentials", "Afternoon Essentials", or "Evening Essentials" — with the matching product set, refreshed on every render. Parsing (`parseTimeBasedFlags()` in `script.js`) is whitespace-safe, case-insensitive, and treats any missing/short/invalid value as `FALSE` rather than erroring, so a blank or malformed cell just means that product is skipped, never a crash.
